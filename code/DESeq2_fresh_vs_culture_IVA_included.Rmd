...
save(dds, comparison001, comparisonALL, 
     file = "/Users/tianyi.li.2/Desktop/STAT_ovary/results/DESeq2_H1_removed_fresh_vs_cultured_IVA_included.Rdata")
```
...
openxlsx::write.xlsx(final, "/Users/tianyi.li.2/Desktop/STAT_ovary/results/tables/DESeq2_fresh_cultured_H1_removed_IVA_included_sig01_20220714.xlsx")
...
kgn <- read_excel_allsheets("/Users/tianyi.li.2/Desktop/STAT_ovary/results/tables/DESeq2_activator_comparison_sig01_20220706.xlsx")
kgn <- kgn[[1]]
...
openxlsx::write.xlsx(gsea, "/Users/tianyi.li.2/Desktop/STAT_ovary/results/tables/GSEA_IVA_specific_hallmark_IVA_included_20230112.xlsx")
...
gse_df <- list()
for (i in 1:length(final_shrunk)) {
  print(paste0("Gene set analysis for"," ", names(final_shrunk)[i]," ", "vs DMSO"))
...
openxlsx::write.xlsx(gsea_hallmark, "/Users/tianyi.li.2/Desktop/STAT_ovary/results/tables/DESeq2_fresh_sigDEGs_IVA_included_GSEA_hallmark_20220714.xlsx")
...
gse <- read_excel_allsheets("/Users/tianyi.li.2/Desktop/STAT_ovary/results/tables/DESeq2_fresh_sigDEGs_IVA_included_GSEA_hallmark_20220714.xlsx")

gse <- gse[1:2]

frag <- gse[[1]] %>% filter(p.adjust < 0.11) %>% 
  select(ID, NES) %>% 
  mutate(group = rep("Fragmentation", nrow(.)))
iva <- gse[[2]] %>% filter(p.adjust < 0.11) %>% 
  select(ID, NES) %>% 
  mutate(group = rep("IVA", nrow(.)))

iva <- iva %>% arrange(desc(NES))
frag <- frag[match(iva$ID[-c(13, 19, 24, 25)], frag$ID),]

gse_all <- rbind(iva, frag)
gse_all <- gse_all %>% mutate(regulate = ifelse(str_detect(NES, "-"), "Down", "Up"))
gse_all$score <- abs(gse_all$NES)
gse_all$score[28:nrow(gse_all)] <- paste0("-", gse_all$score[28:nrow(gse_all)]) %>% as.numeric()
...
act <- read_excel_allsheets("/Users/tianyi.li.2/Desktop/STAT_ovary/results/tables/DESeq2_Activator_all_DEGs_GSEA_hallmark_20220706.xlsx")
